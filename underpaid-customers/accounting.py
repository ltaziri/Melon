...
def expected_pay(log_file):
    cust_file = open(log_file)
    melon_cost = 1.00
    ...
    for line in cust_file:
        ...
        name = customer[1]
        num_melons = float(customer[2])
        paid = float(customer[3])

        expected = melon_cost * num_melons
        if expected != paid:
            if expected > paid:
                under = expected - paid
                print "{} underpaid by ${}".format(name, under)
            else:
                over = paid - expected
                print "{} overpaid by ${}".format(name, over)

expected_pay("customer-orders.txt")
